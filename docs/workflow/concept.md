...
- **[模版 (Template)](/workflow/management/template)**：业务的皮肤。它定义了前端表单结构（Input）以及在门户页的展示形式。
- **[流程 (Workflow)](/workflow/management/workflow)**：业务的骨架。它定义了工单在后台的流转路径、审批逻辑以及自动化任务（Execution）。
