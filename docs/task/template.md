...
| **交付与发布方式** | **内置自动注入 + CLI 热更新**：服务启动自驱注入；支持 CLI 手动发布更新 | **Web 控制台发布**：代码树在线快照或 Webhook 触发打包发布制品 |
...
为了实现**开箱即用（Zero-Config）**，ETask 对系统公共库采用“服务启动自动注入 + CLI 运维按需更新”的双轨机制：

#### 1. 服务启动自驱注入（开箱即用）
平台核心公共库（包括 `third_party/utils/want_result.sh`、Python SDK 基础工具等）已直接内置在 `etask` 服务工程中：
* **自驱检测与物化**：调度中心（Scheduler）启动时，会自动扫描并检测对象存储与元数据库中的系统制品状态；
* **全生态生效**：若未初始化或检测到版本更新，系统自动将内置基础库归档并发布为只读的系统级制品（SYSTEM），所有租户和执行节点即刻开箱即用，无需人工干预。

#### 2. CLI 命令行按需导入（热更新与外部扩展）
当平台管理员需要热更新基础库，或引入新的第三方运维工具包（如 `common-toolkit`）时，可通过运维命令手动导入发布：

```bash
# 导入本地代码目录并发布为 SYSTEM 系统公共组件库
./etask codebook import-system \
  --dir ./scripts/common-toolkit \
  --root-name common-toolkit \
  --replace
```

* `--dir`：需要导入的本地工程目录（必填）；
* `--root-name`：导入后在系统的根目录名称（可选，默认使用本地目录名）；
* `--replace`：可选。若已存在同名系统组件库，先级联清理旧版本再原子导入并发布。
